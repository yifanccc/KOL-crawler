...
from app.models import Asset, KolProfile, RawPost, Signal, SignalAsset, SignalTag, Subscription
...
def _signal_payload(session: Session, signal: Signal) -> dict:
    ...
    subscription = session.get(Subscription, signal.subscription_id) if signal.subscription_id else None
    assets = (
        session.execute(
            select(Asset)
    ...
    signal_tags = (
        session.execute(select(SignalTag.tag).where(SignalTag.signal_id == signal.id)).scalars().all()
    )
    evidence = _json_list(signal.key_points_json or signal.evidence_json)
    kol = session.get(KolProfile, subscription.kol_profile_id) if subscription else None
    ...
    stance_raw = signal.stance if signal.stance in STANCE_TO_CN else STANCE_TO_RAW.get(signal.stance, "unclear")
    ...
    return {
        ...
        "kol": {
            ...
            "displayName": (raw_post.author_name if raw_post else None) or kol.display_name,
            "handle": (raw_post.author_handle if raw_post else None) or kol.display_name,
        ...
        "confidence": confidence_score if confidence_score is not None else signal.confidence or "低",
        "modelConfidence": f"{confidence_score}%" if confidence_score is not None else signal.confidence or "低",
        "promptVersion": signal.prompt_version or "default-v1",
        "ageLabel": "live",
        "tags": tags,
        "evidence": evidence,
        "rawText": raw_post.raw_text if raw_post else None,
        "importance": signal.importance or _importance(signal.confidence, signal.actionable, assets),
        "assets": [asset.symbol if asset.market != "US_STOCK" else f"${asset.symbol}" for asset in assets],
...
def list_signals(
    ...
    base_query = _filtered_signals_query(
        kol_id=kol_id,
        platform=platform,
        requested_asset=asset or symbol,
        tag=tag,
        stance=stance,
...
def get_signal(signal_id: int, db: Session = Depends(get_db)) -> dict:
    ...
    if signal is None:
        return {"item": None}
    return {"item": _signal_payload(db, signal)}
...
def list_assets(db: Session = Depends(get_db)) -> dict:
    rows = db.scalars(select(Asset).order_by(Asset.market, Asset.symbol)).all()
    return {
        "items": [
            {
                "id": row.id,
