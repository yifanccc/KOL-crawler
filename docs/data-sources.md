...
Binance 响应没有稳定订单 ID。provider 使用成交时间、标的、方向、仓位方向、类型、数量和均价生成 `sourceRecordId`，再对可修订成交字段生成 `revision`；所有数值在业务层使用 `Decimal`。`BUY LONG`/`SELL SHORT` 先映射为 `INCREASE`，`SELL LONG`/`BUY SHORT` 映射为 `DECREASE`，本地账本再结合此前推测仓位产生 `OPEN/ADD/REDUCE/CLOSE`。
...
- 推测持仓仅用于解释成交序列，不代表交易所实时仓位；当前 `history_complete=false`，置信度保持保守。
